...
import { Account, AccountRecord } from '../models/account.model';
...
class SalesforceAccounts {
  async getAll(): Promise<Account[]> {
    ...
    const res: QueryResult<AccountRecord> = await conn.query(
      "SELECT name, website, type, description, phone, billingAddress, shippingAddress FROM Account"
    );
  ...
  async get(limit: number, offset: number): Promise<Account[]> {
    const loginUrl = process.env.SF_LOGIN_URL;
    const conn = new jsforce.Connection({
      instanceUrl: process.env.SF_INSTANCE_URL,
      oauth2: {
